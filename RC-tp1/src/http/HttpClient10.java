...
public class HttpClient10 implements HttpClient {
	...
	private static final String GET_RANGE_FORMAT_STR = "GET %s HTTP/1.0\r\nRange: bytes=%d-%d\r\n%s\r\n\r\n";
	private static final String GET_RANGE_END_FORMAT_STR = "GET %s HTTP/1.0\r\nRange: bytes=%d\r\n%s\r\n\r\n";
	...
	public byte[] doGetRange(String urlStr, long start, long end) {
		...
		try {
			...
			int port = url.getPort();
			try (Socket cs = new Socket(url.getHost(), port < 0 ? url.getDefaultPort(): port)) {
				String request = String.format(GET_RANGE_FORMAT_STR, url.getFile(),start, end, USER_AGENT);
				//System.out.println(request);
				cs.getOutputStream().write(request.getBytes());
				return getContents(cs.getInputStream());
			}
	...
	public byte[] doGetRange(String urlStr, long start) {
		...
		try {
			...
			int port = url.getPort();
			try (Socket cs = new Socket(url.getHost(), port < 0 ? url.getDefaultPort(): port)) {
				String request = String.format(GET_RANGE_END_FORMAT_STR, url.getFile(),start, USER_AGENT);
				//System.out.println(request);
				cs.getOutputStream().write(request.getBytes());
				return getContents(cs.getInputStream());
			}
